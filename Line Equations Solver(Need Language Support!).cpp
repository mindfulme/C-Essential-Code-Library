...
int main()
{
	...
	cout << "Select language : ua,en"<<endl;

	cin >> language;
	if (language == "ua") {
		while (ink == 1) {                                                                                         // loop for inputing without exiting command line * Ввод след. без закрытия консоли 
			cout << "Shoise one statement which you need to solve (1-7)" << endl;
			cout << " " << endl;
			...
			case 5:cout << "Give me alpha:";
				...
				cin >> c;// c=p
				if (cos(k / 57.3) / sin(k / 57.3) > 0) { cout << "y=" << c / sin(k / 57.3) << "-" << cos(k / 57.3) / sin(k / 57.3) << "x"; }
				else if (cos(k / 57.3) / sin(k / 57.3) < 0) { cout << "y=" << c / sin(k / 57.3) << cos(k / 57.3) / sin(k / 57.3) << "x"; }
				else { cout << "y=" << c / sin(k / 57.3); }
				break;
